...
const evidenceUsageSnapshot = z.object({
  ...
  cacheReadComparable: z.boolean().optional(), cacheReadPreviousAt: evidenceTimestamp.nullable().optional(),
...
export const providerSessionEvidenceSchema = z.object({
  ...
  session: z.object({
    ...
    contextMachinery: z.object({ observedAt: evidenceNullableTimestamp, model: evidenceText(256), machineryTokens: evidenceCount, contextAllocation: z.object({ initialTokens: evidenceCount, deferredTokens: evidenceCount, reservedTokens: evidenceCount }).strict(), total: z.object({ used: evidenceText(64), limit: evidenceText(64), percentage: z.number().finite().min(0).max(100) }).strict().nullable(), categories: z.array(z.object({ name: evidenceText(128), tokens: evidenceText(64), percentage: z.number().finite().min(0).max(100), kind: z.enum(["initial", "deferred", "reserved"]) }).strict()).max(128), groups: z.array(z.object({ id: evidenceId, label: evidenceText(128), items: z.array(z.object({ name: evidenceText(128), detail: evidenceText(512), tokens: evidenceText(64) }).strict()).max(256) }).strict()).max(128) }).strict().nullable(),
