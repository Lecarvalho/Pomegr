...
7. Return a compact checklist covering policy, automatic loading, delegation coverage, MCP tools, and native session naming. Native naming passes when the main agent can call Pomegr's `rename_session` bridge, or when provider automatic naming is the only available fallback; an idle session may remain Untitled.
