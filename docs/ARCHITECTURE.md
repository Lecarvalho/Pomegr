...
The Codex and Claude plugins share one MCP query registry and local transport. The six
read tools return canonical JSON in `structuredContent` plus a short text fallback. They
...
session failure. Exact session references come from `list_sessions`; `primary` selects
the main agent and delegated IDs come from `list_session_agents`. See
