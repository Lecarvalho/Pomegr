...
The React dashboard opens `/` on the minimalist project home and uses canonical provider-hyphen routes such as `/sessions/[sessionId]` with `claude-…` or `codex-…` segments for a selected session. The home screen reads `/api/home`; the session view polls `/api/state` approximately every 1.8 seconds and refreshes the history sidebar catalog every five seconds. The monitor owns `/api/home` and performs one catalog inspection, independent normalized limit reads for both providers, and one batched live-resource sample, so the browser does not issue provider-specific plan or resource requests. Concurrent home reads share one in-flight build; completed snapshots are served from a short cache, stale snapshots are returned while one refresh runs, and immutable historical summaries are retained for five minutes. A cold build returns live summaries without waiting for recorded-history parsing, marks project history loading, then warms missing historical summaries in a cooperative background pass that yields between transcripts and invalidates the loading snapshot when complete. Home projects exist only when the catalog contains live sessions; their historical aggregates are limited to the last seven days of recorded, completed sessions from those projects. Separately, the live Home limit-activity feed may synchronously inspect a bounded set of recently updated live or completed sessions across repositories for the same provider. It aligns only opaque request timestamps with positive movement between bounded plan observations; it does not sum request usage or claim provider attribution. Claude Code uses its five-hour window. Codex selects the GPT-5.3-Codex-Spark five-hour window only when that model has a unique highest bounded local request-observation count over the preceding seven days; otherwise it selects the general Codex seven-day window. The selector and its model counts remain monitor-private. For Claude Code, the provider adapter may additionally reduce a structured five-hour rejection to its timestamp and reset-window timestamp; the Home API exposes only the earliest matching local rejection timestamp for the current window. When a zero-percent snapshot omits its reset timestamp, the monitor carries the previous reset boundary across the transition or recovers it from normalized local rejection evidence, and marks whether the resulting window start is exact. A selected historical session is read once because its transcript is immutable for dashboard purposes. Home and live-state reads ask the monitor for provider plan usage, but only the monitor decides whether a provider request is due. Each provider shares one in-flight request and one cache across every browser connected to that service process, refreshes at most once every five minutes, and extends the cooldown when the provider returns `Retry-After`. Cached values are returned immediately while a refresh is already running. The server routes proxy to the private monitor so remote browsers never receive credentials, account data, or raw transcripts.
