...
The dashboard window uses a minimal preload, renderer sandboxing and context isolation, with Node integration, worker/subframe Node integration, webviews, insecure content, and experimental renderer features disabled. Permission checks and requests, device access, Chromium downloads, webview attachment, unexpected navigation, redirects, and new windows are denied. Only the Pomegr source and license paths under the fixed project GitHub URL may open in the external browser. The desktop response carries a restrictive same-origin CSP and frame, referrer, opener, and content-type protections. Renderer IPC is fixed to report export plus bounded desktop settings, pause/quit actions, state updates, and native-theme synchronization; every main-process handler validates the exact local sender origin. Report export additionally validates a generated filename grammar, a fixed report header, exact request keys, and a 2 MiB UTF-8 limit before showing the native save dialog. Native theme input is allowlisted to `light`, `dark`, or Electron's `system` value. The currently exposed light/dark renderer preference promptly synchronizes Electron's process-global native theme so the standard Windows title bar matches the single shell window; the fixed dark startup-error page explicitly selects matching dark native chrome. The renderer never receives filesystem paths or arbitrary Electron, filesystem, process, or environment access.
