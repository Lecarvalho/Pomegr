...
Safe cost snapshots ────┘            │
                                     │ normalized JSON
                                     v
          /api/sessions + /api/session-domain + /api/session-history + /api/state + supporting proxies
                                     │
                                     v
                           Pomegr web UI (:3003)
```
...
Electron main also creates a separate 256-bit agent-query capability for each launch and
passes it only to the monitor worker. After monitor startup it atomically publishes a
bounded rendezvous descriptor in the stable per-user Pomegr data root so installed and
portable launches are discoverable by packaged MCP clients. That capability
authorizes only `GET /api/agent/v1/*`; it cannot authorize ordinary monitor APIs or the
...
9. Derives and atomically publishes seven browser-safe session domains from committed
   evidence: `session-summary`, `agents`, `agent`, `signals`, `repository`, `resources`,
   and `details`. Each has independent readiness and a monotonic revision. The composed
   `/api/state` response remains during browser migration.
...
read tools return canonical JSON in `structuredContent` plus text content: ordinary
queries use a short fallback, while `get_session_report` returns its Markdown report. They
are passive and decision-triggered: they do not launch Pomegr, refresh provider state,
parse a transcript, send control messages, or infer that a public incident caused a
session failure. Session-scoped reads default to one validated host-provided stdio MCP
session identity; missing or ambiguous identity fails unavailable rather than guessing
from a directory, recency, or process activity. `get_agent_context` defaults to `primary`,
while optional exact session and delegated-agent references come from `list_sessions` and
`list_session_agents`. `get_session_report` has no arguments and returns the same bounded
Markdown generated by the dashboard from one committed session revision. See
[MCP observation queries](MCP_QUERIES.md) for the interface and metric limits.
...
The React dashboard opens `/` on personal Home and uses canonical provider-hyphen
session routes, such as `/sessions/claude-…`. `AppShell` owns the committed catalog
snapshot; Sessions renders its bounded summaries without a separate browser-side join.
Home resolves local pinned identifiers and the last-viewed session against that catalog.

One reference-counted `app/live-events.ts` stream supplies the shell, session, history and
repository consumers. Events trigger serialized revision-gated GETs with one coalesced
follow-up for an in-flight invalidation. Session/catalog/history/repository recovery uses
30 seconds connected, 5 seconds reconnecting and 30 seconds hidden. Live loading may
poll every second. Historical queries finish hydration through events and reconnects,
then stop periodic polling. `/api/state` remains the composed compatibility view while
future tabs adopt the independent domains. The exact query must have a retained body
before its revision can produce a usable `204`; uncached navigation and preload queries
fetch full bodies. Pinned request selection stays on its committed visible page while
new history totals extend the minimap. The full cadence and readiness contract is in
[Observation cache](OBSERVATION_CACHE.md#frontend-api-cadence).

Browser presentation timing is not collected by operations diagnostics and never becomes
normalized evidence or a product metric. Existing committed values remain rendered during
refresh and failures. Desktop Pause affects presentation only.
...
Focused Markdown observation reports are assembled from the same committed normalized state. The monitor precomputes bounded report evidence before dashboard detail caps; the renderer exports the latest bounded provider-reported model and reasoning effort per normalized agent, selected refill transitions, compaction/context boundaries, retained task failures, supporting request snapshots, and explicit coverage instead of scores or recommendations. Missing runtime values remain unavailable, and the report does not infer model history, service tier, routing, performance, or cost. Report generation refreshes the cache-only local state endpoint, does not call a model, and does not request the provider usage endpoint. Browser development uses a normal download; desktop uses a narrow IPC method that accepts only a bounded Pomegr Markdown report and opens the operating-system save dialog after the user clicks **Generate report**.

### Development orchestrator

`scripts/dev.mjs` starts both long-running processes. The development monitor constructs its anonymous continuous JSONL diagnostics before beginning observation, with no capture IPC listener in the normal path; the web process binds to `0.0.0.0:3003` for local-network viewing.
...
The dashboard window uses a minimal preload, renderer sandboxing and context isolation, with Node integration, worker/subframe Node integration, webviews, insecure content, and experimental renderer features disabled. Permission checks and requests, device access, Chromium downloads, webview attachment, unexpected navigation, redirects, and new windows are denied. Only the Pomegr source and license paths under the fixed project GitHub URL may open in the external browser. The desktop response carries a restrictive same-origin CSP and frame, referrer, opener, and content-type protections. Renderer IPC is fixed to report export plus bounded desktop settings, pause/quit actions, update checks and installation, state updates, and native-theme synchronization; every main-process handler validates the exact local sender origin. Update state contains only the bounded status enum, validated semantic version, and last successful check timestamp; the desktop snapshot separately includes the validated installed application version. Both check and install methods take no renderer arguments, remain native-only, and never accept a release URL, path, or command. Checks share in-flight work and are unavailable during downloads, installation, or when a verified update is already ready. Check timestamps remain in memory and failures do not advance them. Report export additionally validates a generated filename grammar, a fixed report header, exact request keys, and a 2 MiB UTF-8 limit before showing the native save dialog. Native theme input is allowlisted to `light`, `dark`, or Electron's `system` value. The currently exposed light/dark renderer preference promptly synchronizes Electron's process-global native theme so the standard Windows title bar matches the single shell window; the fixed dark startup-error page explicitly selects matching dark native chrome. The renderer never receives filesystem paths or arbitrary Electron, filesystem, process, or environment access.
