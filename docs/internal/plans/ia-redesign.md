...
- Tab bar sits directly under the KPIs. Tabs in order: Overview, Agents, Activity, Repository, Requests & cache, Resources, Details. Resources is hidden only when a session has neither live samples nor stored resource rows. Tab is a URL query (`?tab=`) and deep links accept `agent`, `request`, and `path` parameters.
...
- **Activity row to Requests deep link (T04b, artboard `RequestsFromActivity`).** Drawn but not approved. Concern raised on 2026-09-13: clicking an Activity row jumps to the Requests tab; clicking a bar there would send the reader back to Activity, and the two tabs become a ping-pong. Alternatives to weigh: show the request detail inline on the Activity tab (an expandable row or a right-pane detail that replaces Shell tasks while a row is selected) and keep the Requests tab for the chart; or make Requests the only place with detail and have Activity rows show a small request badge without navigation. Decide before T04b starts; the artboard stays as one option, not the decision.
