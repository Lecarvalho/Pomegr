...
Home live cards derive from the shared five-second `{ sessions, liveSessions }` response owned by `AppShell`, rather than from the aggregate-only `/api/home` response. `sessions` drives the sidebar plus Dashboard selection and attention state, while bounded `liveSessions` drives home cards. `liveSessions` exposes only bounded session status/progress, agent counts, and latest all-agent context. The browser polls `/api/home` every 30 seconds for aggregate data; it excludes projects, live cards, and history, although the monitor may retain a fuller cached snapshot and coalesce its summary parsing. The home monitor performs one catalog inspection and one batched resource sample; resources are in-memory telemetry and are never reconstructed for history.
...
A possible-full-refill occurrence receives a cache-expiry inference only when the current request carries the normalized `previous_cache_entry_unavailable` provider status, has no recognized direct divergence reason, and its preceding comparable request has a resolved lifetime whose full threshold has elapsed. The threshold is five minutes for `5m`, one hour for `1h`, and one hour for `mixed` because the longest-lived portion must expire before a full-prefix expiration is supported. Model changes, comparison-group changes, intervening compactions, missing status, missing lifetime, and shorter gaps fail closed. The dashboard labels this derived explanation **Inference** and continues to show the provider status separately.
