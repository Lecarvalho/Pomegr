...
Home live cards derive from the shared revisioned `{ sessions, liveSessions }` catalog response owned by `AppShell`, rather than waiting for Home correlation. `sessions` drives the sidebar plus Dashboard selection and attention state, while bounded `liveSessions` drives Home cards and exposes only bounded session status/progress, agent counts, and latest all-agent context. Provider/account usage comes from the centralized revisioned `/api/usage-limits` cache; `/api/home` supplies derived local activity and the provider-limit revision it matched. A mismatched or unresolved correlation lane stays a skeleton while an already-ready usage bar renders. Loading Home polls once per second, ready Home with live sessions every five seconds, and ready Home without live sessions every 30 seconds; hidden tabs use 30 seconds. Resource sampling and Home derivation run monitor-side rather than inside browser GET handlers, and resources are never reconstructed for history.
