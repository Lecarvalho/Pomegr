...
Home live cards derive by filtering the shared revisioned `{ sessions }` catalog response owned by `AppShell`, rather than waiting for Home correlation. Each presentation-ready row exposes only bounded session status/progress, agent counts, latest all-agent context, summary readiness, and live primary-agent activity. The same row drives sidebar and Dashboard selection, attention state, the Sessions directory, and Home; React performs no identity/summary join. Completed rows retain their final agent count, latest context snapshot, and recorded progress. Provider/account usage comes from the centralized revisioned `/api/usage-limits` cache; `/api/home` supplies derived local activity and the provider-limit revision it matched. A mismatched or unresolved correlation lane stays a skeleton while an already-ready usage bar renders. Loading Home polls once per second, ready Home with live sessions every five seconds, and ready Home without live sessions every 30 seconds; hidden tabs use 30 seconds. Resource sampling and Home derivation run monitor-side rather than inside browser GET handlers, and resources are never reconstructed for history.
...
Agent activity renders each agent's authoritative cache lifetime alongside its runtime metadata in List view and as evidence in Tree view. It renders an amber stack-refill symbol from the per-agent refill summary. Tree clusters sum only the bounded counts belonging to agents represented by the cluster. The wording remains cautious because the transition alone does not prove expiration, eviction, a full-prefix rewrite, or billing impact.
