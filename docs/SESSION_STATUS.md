...
| **Open** (`open`) | **Codex** | No Needs input, In progress, Idle, or Stopped rule applies, and at least one live agent has confirmed runtime/owner presence. | Explicit owning-runtime observation, or a valid version-two lifecycle bridge with current owner-backed presence. | Presence is known while execution state is unresolved. Claude has no global Open classification; a comparable registered session without usable execution evidence generally remains Unknown. |
...
| Codex | At least one root/related agent is live under its selected lifecycle evidence: owning-runtime presence, a lifecycle lease (including bounded expiry grace), or unresolved structured execution/input evidence. | Recorded starts can remain active through silence. A recorded completed turn alone is non-live; owner evidence can keep an idle agent live. Lease-backed presence and recorded execution are distinct facts. |
...
| Lifecycle hook bridge | Codex | Used when valid bridge records exist. User/subagent starts and recognized input/approval hooks can supply state. Stop, SubagentStop, and SessionEnd hooks produce uncertainty rather than definitive completion. Leases last 45 seconds; expired, old, or ambiguous evidence cannot supply a fresh execution state. |
| Owning-runtime status and approval flags | Codex | The adapter can consume an explicitly supplied owning connection, but the default production registry does not supply one. Its separate app-server reader is account-rate-limits-only and cannot supply session status or approval waits. Runtime observations are discarded on read failure or after more than 120 seconds without confirmation. |
| Legacy activity/approval/plan inference | Codex | General inference requires every deterministic channel (owning runtime, hooks, structured rollout) to be explicitly declared unsupported. The default configuration does not enable that gate. The heuristic code's existence does not establish production coverage for mobile approvals, CLI pending edits, or plan-confirmation waits. Recognized structured `request_user_input` has a separate accepted-evidence path. |

Sources: [default integrations](../monitor/providers/index.mjs), [Claude lifecycle reader](../monitor/providers/claude-session-status.mjs), [Codex owning runtime](../monitor/providers/codex-owning-runtime.mjs), [Codex hooks](../monitor/providers/codex-hook-lifecycle.mjs), [Codex inference gate](../monitor/providers/codex-source-routing.mjs).
