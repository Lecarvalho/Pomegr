...
- [ ] Every provider in the snapshot renders one plugin row and one inventory row; the
      reporting row renders once.
- [ ] Primary buttons appear only for Install plugin, Update plugin, and Run diagnostic.
- [ ] Native actions still go through the desktop bridge; browser clients see
      instructions, never a disabled primary button.
- [ ] Feedback messages and pending/cancelled/busy/failed copy are unchanged from today.
...
- [ ] Deep link `?tab=inventory&provider=claude&revision=ctx-001` opens the tab with that
      revision selected.
- [ ] Category grid, item groups, and revision comparison behave exactly as before.
- [ ] Failure kinds render their existing sanitized messages; no raw output anywhere.
...
- [ ] No horizontal overflow at 360 px on either view; all controls 44 px on phone.
- [ ] Keyboard-only navigation reaches every tab, row link, and action.
- [ ] No selector left in `workspace.css` that nothing renders.
- [ ] `DESIGN.md` describes the shipped structure.
