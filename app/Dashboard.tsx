...
function SessionKpis({ summary, historical }: { summary: SessionSummaryDomain; historical: boolean }) {
  ...
  return <section className="sessionKpiStrip sessionSummaryKpis" aria-label="Session totals">
    <div className="sessionKpi"><span className="sessionEyebrow">Agents</span><strong>{agentsReady ? summary.metrics.agents.toLocaleString() : "—"}</strong><small>{statusCountsReady ? <><span className={summary.metrics.activeAgents ? "sessionPositive" : undefined}>{summary.metrics.activeAgents} active</span> · {summary.metrics.idleAgents} idle · {summary.metrics.finishedAgents} finished</> : "Agent status counts unavailable"}</small></div>
    <div className="sessionKpi"><span className="sessionEyebrow">All-agent context</span><strong className="sessionContextValue">{contextReady ? compactNumber(summary.allAgentContext) : "—"}</strong><small>{contextReady ? "Latest snapshots · not spend" : "Context evidence unavailable"}</small></div>
