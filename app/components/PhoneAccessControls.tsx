...
export function PhoneAccessControls() {
  ...
  const enabled = isSharing || recovering;
  ...
  const shareDisabled = state.status === "starting" || busy !== null || (!enabled && needsNetwork && !selectedNetwork);
  ...
  return <section className="phoneAccessControls" aria-labelledby="phone-access-heading">
    ...
    {needsNetwork && <label className="phoneAccessNetwork"><span>Private network</span><CommandSelect aria-label="Private network" value={selectedNetwork} disabled={busy !== null || enabled || state.status === "starting"} onChange={(event) => setSelectedNetworkId(event.currentTarget.value)}><option value="">Choose a private network</option>{state.candidates.map((candidate) => <option value={candidate.id} key={candidate.id}>{candidate.label} · {candidate.address}</option>)}</CommandSelect></label>}
    <div className="phoneAccessAction"><div><strong>{recovering ? "Reconnecting phone access" : isSharing ? "Sharing started" : state.status === "starting" || busy === "sharing" ? "Starting phone access" : unavailable ? "Phone access needs attention" : "Phone access is off"}</strong><span>{recovering ? "Checking your private network. Paired phones will reconnect automatically." : isSharing && address ? <>Open <a href={address}>{address}</a> on a paired phone.</> : unavailable ? unavailableGuidance(state) : "Phone sharing is off by default."}</span></div><div className="phoneAccessButtons">{unavailable && <button className="commandSecondaryAction" type="button" disabled={busy !== null} onClick={() => void refreshNetworks()}>{busy === "refresh" ? "Refreshing…" : "Refresh networks"}</button>}<button className={enabled ? "commandSecondaryAction" : "commandPrimaryAction"} type="button" disabled={shareDisabled} onClick={() => void changeSharing(!enabled)}>{busy === "sharing" ? (enabled ? "Stopping…" : "Starting…") : enabled ? "Stop sharing" : unavailable ? "Retry phone access" : "Enable phone access"}</button></div></div>
