...
export function RequestCacheDetail({ row }: { row: RequestRow }) {
  ...
  const definition = inferred ? cacheReadReuseDroppedSignalDefinition() : occurrence ? cacheRefillSignalDefinition(occurrence) : null;
  return <section className="requestsActionsCacheDetail" aria-label="Request cache evidence">
    <h4><CacheRefillIcon inferred={inferred} />{cacheEvidenceLabel(evidence)}</h4>
    <p className="requestsActionsCacheProvenance">{inferred ? "Inference · no recorded cache write" : "Recorded cache-write evidence"}</p>
    <dl>
      {currentRead !== null && <div><dt>Cache read</dt><dd>{previousRead != null ? `${previousRead}% → ` : ""}{currentRead}%</dd></div>}
      {!inferred && <div><dt>Cache write</dt><dd>{(evidence.event?.cacheWriteTokens ?? row.cacheWriteTokens).toLocaleString()} tokens</dd></div>}
      {observation?.gapMs != null && <div><dt>Request gap</dt><dd>{formatDuration(observation.gapMs)} wall time</dd></div>}
    </dl>
    {inferred ? <p>No positive cache-write evidence, so a refill and its cause cannot be confirmed.</p> : <>
