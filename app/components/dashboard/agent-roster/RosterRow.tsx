...
import { agentAssignment, agentDisplayLabel, agentDisplayName, agentRoleLabel, cacheLifetimeLabel, compactNumber } from "../../../dashboard-utils";
...
export function RosterRow({ agent, depth, selected, onSelect, insights, loops, executionTasks, cacheRefills, cacheReadDrops, contextBoundaries, rowRef }: {
  ...
  return <div ref={rowRef} role="row" aria-label={`${agentDisplayLabel(agent)} agent, ${cacheLifetimeLabel(agent.cacheLifetime)}`} aria-selected={selected} className={`rosterRow ${agent.id === "primary" ? "rosterPrimary" : ""} ${selected ? "rosterSelected" : ""}`} style={{ "--agent-indent": `${Math.min(depth, 8) * 12}px` } as CSSProperties}>
    ...
    <div className="rosterIdentity">
      ...
      <span className="rosterMeta rosterDesktop">{agentAssignment(agent) && <span className="agentMetaIdentity">{agent.label} · </span>}{agentRoleLabel(agent, agent.role)} · {agent.model} · {agent.effort} · {agent.skills.length > 0 ? `${agent.skills.length} skills · ` : ""}{shellCount} shell tasks</span>
      <span className="rosterMeta rosterPhone">{agentAssignment(agent) ? `${agent.label} · ` : ""}{agentRoleLabel(agent, agent.role)} · {agent.model} · {agent.toolCalls} calls</span>
    </div>
