...
export function RepositoryDetailView({ repositoryId, initialTab = "overview", initialProvider, initialRevisionId }: {
  ...
  return <section className="commandView repositoryDetail" aria-label={repository.displayName}>
    <CommandPageHeader className="repositoryDetailHeader" breadcrumb={<><Link href="/repositories">Repositories</Link><CommandBreadcrumbSeparator /><span aria-current="page">{repository.displayName}</span></>} meta={<div className="repositoryDetailMeta"><span className="repositoryDetailCount"><strong className={repository.liveCount > 0 ? "live" : undefined}>{repository.liveCount}</strong> live</span><span aria-hidden="true">·</span><span className="repositoryDetailCount"><strong>{repository.historyCount}</strong> history</span>{repository.providers.some((provider) => provider.sessionCount > 0) && <span aria-hidden="true">·</span>}{repository.providers.filter((provider) => provider.sessionCount > 0).map((provider) => <ProviderBadge key={provider.provider} source={provider.source} />)}</div>} actions={<Link className="commandSecondaryAction" href={`/sessions?repository=${repository.id}`}>View sessions <CommandIcon name="arrow" size="small" /></Link>} />
