...
import { ContextAllocationBreakdown, inventoryDisplayTokens } from "../context-inventory/ContextAllocationBreakdown";
...
export function RevisionEvidence({ repository, provider, selectedRevisionId, onSelect }: {
  ...
  return <div className="repositoryInventoryEvidence">
    <section className="repositoryInventorySummary" aria-label={`${provider.source} inventory summary`}>
      ...
      <div className="repositoryInventoryInitial"><span>{detail.contextAllocation ? "Estimated initial context" : "Categorized total"}</span><strong className="repositoryInventoryTokens">{compactNumber(inventoryDisplayTokens(detail))} tokens</strong></div>
    </section>
    ...
    {detail.contextAllocation ? <ContextAllocationBreakdown allocation={detail.contextAllocation} categoryTotal={detail.machineryTokens} categories={detail.categories} /> : <p className="repositoryInventoryLegacy">This saved reference predates the context allocation split. Capture again to distinguish initial, deferred, and reserved tokens.</p>}
    {detail.groups.length > 0 && <details className="repositoryInventoryDetails"><summary><span>Inspect {detail.itemCount} provider-listed items</span><small>Partial detail, not a sum of the categorized total</small></summary><div className="repositoryInventoryGroups">{detail.groups.map((group) => <section key={group.id}><h4>{group.label}</h4>{group.items.map((item, index) => <div className="repositoryInventoryItem" key={`${item.name}-${index}`}><span><strong>{item.name}</strong><small>{item.detail}</small></span><b>{item.tokens}</b></div>)}</section>)}</div></details>}
    {provider.revisions.length > 1 && <details className="repositoryInventoryDetails"><summary>Compare revisions</summary><div className="repositoryRevisionCompare"><label>Revision<CommandSelect value={selectedRevisionId} onChange={(event) => onSelect(event.currentTarget.value)}>{provider.revisions.map((revision) => <option value={revision.id} key={revision.id}>{revision.id} · {compactNumber(inventoryDisplayTokens(revision))}</option>)}</CommandSelect></label>{current && current.id !== detail.id && <dl><div><dt>Initial context</dt><dd>{detail.contextAllocation && current.contextAllocation ? `${compactNumber(detail.contextAllocation.initialTokens - current.contextAllocation.initialTokens)} vs current` : "Unavailable"}</dd></div><div><dt>Categories</dt><dd>{detail.categoryCount - current.categoryCount} vs current</dd></div><div><dt>Listed items</dt><dd>{detail.itemCount - current.itemCount} vs current</dd></div></dl>}</div></details>}
