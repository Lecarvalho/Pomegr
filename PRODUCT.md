...
- Cache events are bounded, monitor-derived, agent-attributed observations from provider-comparable usage evidence. They expose only normalized refill, reuse, and cautious miss-refill metadata plus recognized bounded provider-diagnostic cause categories when present. A documented structural lifecycle rule may add a fixed, clearly labeled Pomegr inference about which tool definitions changed; incomplete or ambiguous evidence remains unavailable. Raw diagnostics, lifecycle records, schemas, usage records, bills, charges, and savings estimates remain private.
