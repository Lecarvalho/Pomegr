...
test("clean-VM upgrade fixture is isolated, test-only, and preserves candidate metadata", async () => {
  ...
  assert.match(checklist, /Pomegr-Setup-0\.2\.4-x64\.exe/);
  assert.match(checklist, /Pomegr-Portable-0\.2\.4-x64\.exe/);
  assert.match(checklist, /SmartScreen/);
  assert.match(checklist, /POMEGR-DT-08/);
  assert.match(checklist, /Acceptance status\s+\*\*PENDING:/);
  assert.match(checklist, /contains no Pomegr 0\.2\.4 PASS claim/);
  assert.match(checklist, /prior-only input, not Pomegr 0\.2\.4 proof/);
