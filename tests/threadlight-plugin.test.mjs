...
import { buildThreadlightMcpServer } from "../plugins/claude-code/mcp/server.mjs";
import {
  findPolicy,
  POLICY_MAX_BYTES,
  readPolicy,
...
async function withTemporaryDirectory(run) {
  const directory = await mkdtemp(path.join(os.tmpdir(), "threadlight-plugin-"));
  try {
...
async function writePolicy(repository, text) {
  const directory = path.join(repository, ".threadlight");
...
test("rejects policies that contradict naming, privacy, and signal-lifetime invariants", async () => {
  ...
  assert.ok(badPrivacy.errors.includes("Privacy and semantics must match the canonical Threadlight safety policy."));
...
test("rejects a non-regular policy path before reading or injecting it", async () => {
  await withTemporaryDirectory(async (temporaryRoot) => {
    ...
    await mkdir(path.join(repository, ".threadlight", "signals.md"), { recursive: true });
    ...
    assert.match(JSON.parse(hook.stdout).systemMessage, /\/threadlight:doctor/);
  });
});
...
test("finds a policy upward only as far as the repository root", async () => {
  await withTemporaryDirectory(async (temporaryRoot) => {
    ...
    assert.equal(missing.path, path.join(repository, ".threadlight", "signals.md"));

    const template = await readFile(policyTemplatePath, "utf8");
    await writePolicy(repository, template);
    const found = findPolicy(nested);
    assert.equal(found.repositoryRoot, repository);
    assert.equal(found.path, path.join(repository, ".threadlight", "signals.md"));
...
test("SessionStart hook injects valid policy context, stays silent when missing, and warns safely when invalid", async () => {
  await withTemporaryDirectory(async (temporaryRoot) => {
    ...
    assert.match(validOutput.hookSpecificOutput.additionalContext, /\[Threadlight reporting policy loaded\]/);
    assert.match(validOutput.hookSpecificOutput.additionalContext, /allow Claude Code to assign/i);
    assert.match(validOutput.hookSpecificOutput.additionalContext, /# Threadlight reporting policy/);
    ...
    assert.match(invalidOutput.systemMessage, /\/threadlight:doctor/);
...
test("plugin manifests register every SessionStart transition and the self-contained MCP server", async () => {
  ...
  assert.equal(manifest.name, "threadlight");
  ...
  assert.match(mcp.mcpServers.threadlight.args[0], /\$\{CLAUDE_PLUGIN_ROOT\}/);
  ...
  assert.match(doctor, /\[Threadlight reporting policy loaded\]/);
...
test("plugin MCP inventory contains only reporting and scoped clearing tools", () => {
  const server = buildThreadlightMcpServer();
  ...
  assert.equal(tools.includes("ask_threadlight"), false);
