...
const POSIX_FAIL_CLOSED = [
  ...
  ["pushd before a redirect", "pushd sub > log.txt && echo hi > out.txt"],
  ["popd before a redirect", "popd > x && touch a"],
  ["cd behind builtin", "builtin cd sub && touch a"],
  ["a comment hiding the rest of the line", "echo hi # > f"],
  ["an unquoted backslash escape", "touch src\\a.ts"],
  ["home expansion", "echo x > ~/notes.txt"],
  ["brace expansion", "touch {a,b}.txt"],
  ["a subshell", "(touch a.txt)"],
...
const POWERSHELL_FAIL_CLOSED = [
  ...
  ["Set-Location before a redirect", "Set-Location sub > log.txt; Set-Content a.txt x"],
  ["Push-Location anywhere", "Push-Location sub; Remove-Item a.ts"],
  ["a sub-expression argument", "Remove-Item (Join-Path src old.ts)"],
  ["an array argument", "Remove-Item a.txt,b.txt"],
  ["a splat", "Remove-Item @targets"],
  ["a script block", "Set-Content -Path a.txt -Value {hi}"],
