...
test("uses one provider-neutral identity and no starter preview", async () => {
  ...
  assert.match(styles, /\.firstRequestStat/);
  assert.match(monitor, /readFirstRequestFootprint\(mainFile\)/);
  assert.match(dashboard, /View loaded machinery/);
  assert.match(dashboard, /Run \/context to load machinery/);
  ...
  assert.match(styles, /\.firstRequestStat \{ position: relative/);
