...
test("uses one provider-neutral identity and no starter preview", async () => {
  ...
  assert.match(dashboard, /agentTaskTrigger/);
  assert.match(dashboard, /agentTaskPopover/);
  assert.match(dashboard, /PRIMARY ORCHESTRATION/);
  assert.match(dashboard, /Blocked by/);
  assert.match(styles, /\.agentTaskPopover/);
  assert.match(styles, /\.agentTaskRow\.completed/);
  assert.match(monitor, /readSessionTasks\(TASKS_ROOT, sessionId\)/);
