...
- Session-signal metadata may expose only a bounded plain-text label, semantic tone, and transcript-derived timestamp from a recognized Threadlight MCP tool call. Agent-signal metadata may additionally expose an optional bounded, one-line plain-text description for its tooltip. Never expose other MCP arguments or tool-result content, and present signals as agent-reported rather than Threadlight judgments.
