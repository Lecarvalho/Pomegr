...
- Internal pipeline-operations diagnostics remain read-only, bounded, and outside browser API state. Aggregate observations stay in memory. The development entrypoint may continuously retain bounded timing slices, outcomes, counters, and opaque correlation handles in a rolling memory buffer. Authenticated local IPC may save recent history to a user-selected local file without clearing or stopping the recorder. As explicitly authorized for session-focused local investigation, existing normalized session associations may remain privately in a bounded development-only lookup outside the recorder solely to select retained work; the local caller supplies its already-known normalized selector. Only opaque recorder-minted scope objects may enter trace APIs. Never return or persist that selector, the association map, session/source identity, or raw provider data in trace events, reports, descriptors, logs, or browser telemetry. Exports retain anonymous correlation handles and applicable shared work only. Diagnostic reads must never trigger provider acquisition, normalization, derivation, checkpoint persistence, or product publication. Recorder, capture transport, and renderer instrumentation must be excluded from production and desktop builds, not merely runtime-disabled. See `docs/PIPELINE_OPERATIONS.md`.
