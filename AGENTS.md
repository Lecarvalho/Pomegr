...
- Codex lifecycle-hook bridges may persist only bounded normalized session/turn/agent identifiers, an allowlisted lifecycle event enum, `SessionStart` source enum, `stop_hook_active` boolean, timestamp, sequence, and local lease state. Never persist or expose raw hook payloads, prompts, transcript paths, commands, hook output, or unrecognized fields; Stop/SubagentStop/SessionEnd and compact SessionStart are boundary evidence, not definitive completion or idle.
