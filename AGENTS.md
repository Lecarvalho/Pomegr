...
- Internal pipeline-operations diagnostics remain read-only, bounded, and outside browser API state. Aggregate observations stay in memory. Explicit trusted local diagnostic capture may additionally record bounded timing slices, outcomes, counters, and fresh capture-local correlation tokens and export an allowlisted trace to a user-selected local file. Capture is disabled by default, authenticated through local IPC, and time/event bounded; browser/LAN routes cannot activate it. A diagnostic client must never trigger provider acquisition, normalization, derivation, checkpoint persistence, or product publication, and must never receive source/session identity or raw provider data. See `docs/PIPELINE_OPERATIONS.md` for recording, privacy, and offline analysis rules.
- Browser `performance.mark()` telemetry is an opt-in F Presentation diagnostic, not normalized session evidence or a product metric. It may use only fixed allowlisted marks and capture-local revision tokens minted after committed responses, with bounded revision-correlated durations under the privacy and activation contract in `docs/PIPELINE_OPERATIONS.md`. Browser or LAN traffic cannot enable capture, mint arbitrary associations, provide a clock claim, paths, URLs, text, source/session/native IDs, or output destinations. A token-bearing request may return only its fixed browser-monotonic start/response interval; the monitor derives and exports only a bounded request-interval error, never raw browser timestamps or a browser-selected offset.
