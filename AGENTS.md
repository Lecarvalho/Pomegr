...
- Provider-native agent kinds and repository role-map contents are monitor-private. The browser receives the bounded normalized `Agent.role` enum and, only for an unmapped role, a monitor-validated `Agent.customType` terminal identifier displayed as `custom: <type>`. Validate the entire source identifier before normalization; reject paths, prose, markup, control characters, and values over 64 characters. Never expose the full provider kind or mapping contents, or reinterpret the custom label as a role. See `docs/METRICS.md`.
- Claude status-line cost capture may persist and expose only the normalized session ID, non-negative `total_cost_usd`, USD currency label, estimate type, and local observation timestamp. Always present the value as a Claude Code estimate rather than authoritative billing. The separate local usage feed may persist only a version, local observation timestamp, and a complete pair of normalized five-hour/seven-day percentages and reset timestamps. Never persist any other status-line fields. Expose only normalized usage windows, bounded origin/freshness enums, and the original observation time; repeated identical output must not renew freshness.
- Claude account-usage recovery may separately persist only a version, opaque credential-source filesystem fingerprint, original successful-observation and last-attempt timestamps, bounded failure kind, retry deadline, and the three recognized normalized usage windows (fixed ID, percentage, reset timestamp, active flag). Restore only for the same credential source; reconstruct labels/severity and never persist credential contents, credential paths, account IDs, raw responses, headers, or error text. Keep the status-line pair separate, preserve original ages and provider cooldowns across restarts, and never expose this cache or its source fingerprint through browser state. See `docs/OBSERVATION_CACHE.md`.
...
- Execution-task metadata may expose only normalized tool/background IDs, the Bash description, shell kind, a bounded provider-neutral work kind, lifecycle status, timestamps, background flag, exit code, and a bounded enum-based failure category derived monitor-side. Work kind must come from a recognized structured tool identity or private command structure; ambiguous work remains generic. Never expose commands, stdout, stderr, matched source text, or task-notification output.
- Activity metadata may additionally expose only a nullable bounded call-to-result wall duration (maximum 24 hours) and an opaque normalized request ID. The paged session-history surface may expose normalized agent IDs and stable session-scoped request numbers, including links outside the bounded state summary. Its committed disk history and indexes contain only allowed normalized metadata; GETs never acquire provider evidence. Work-kind aggregates contain only bounded WorkKind counts and median resolved durations. Provider call/message IDs and private request mappings remain monitor-private; no per-action token or cost attribution is permitted. See `docs/OBSERVATION_CACHE.md`.
...
- Codex native writer presence is read-only and monitor-private. On Windows, confirm byte-range contention, stable file identity, a unique native process owner, an allowlisted executable path, and matching process-start identity before asserting presence. Retain only bounded in-memory ownership and confirmation state; never persist or expose lock paths, executable paths, PIDs, process identities, helper output, or errors in browser state. Ownership establishes presence, not executing work or completion. Do not consume lifecycle hook snapshots or leases, write provider files, attach a worker, or control provider processes. Other platforms require separately validated native semantics; no plugin fallback is implied.
...
- Current repository plugin setup is a separate read-only observation of local installation records, resolved manifests, and effective provider configuration. It may expose only bounded installation/readiness/enablement/scope enums, installed and available versions, original check times, update status, action availability, and shared reporting-policy status/version. It must never replace session-observed plugin metadata. Raw configuration, paths, provenance, and policy content remain private. Plugin install/update actions require an explicit native desktop confirmation, revalidated official source/scope/version plan, fixed CLI arguments, and a fresh local verification. Browser/LAN GETs cannot trigger these actions. See `docs/OBSERVATION_CACHE.md`.
- Current repository plugin setup is a separate read-only observation of local installation records, resolved manifests, and effective provider configuration. It may expose only bounded installation/readiness/enablement/scope enums, installed and available versions, original check times, update status, action availability, and shared reporting-policy status/version. It must never replace session-observed plugin metadata. Raw configuration, paths, provenance, and policy content remain private. Plugin install/update actions require an explicit native desktop confirmation, revalidated official source/scope/version plan, fixed CLI arguments, and a fresh local verification. Browser/LAN GETs cannot trigger these actions. See `docs/OBSERVATION_CACHE.md`.
...
- Agent cache-lifetime metadata may expose only the aggregate `5m`, `1h`, `mixed`, `30m+`, or `null` derived from every retained, resolved request lifetime for that normalized agent. `30m+` is a documented model-policy minimum, rendered `≥30m`, never a recorded expiry or an expiry-inference threshold. The aggregate is per agent, never session-wide or latest-only. Raw provider cache-control blocks and token breakdowns remain monitor-private.
...
- Request-snapshot metadata may expose only a bounded opaque ID, normalized agent ID, observation timestamp, the resolved `5m`/`1h`/`mixed` cache lifetime, documented `30m+` minimum, or `null`, normalized request-local uncached-input, cache-write, cache-read, and output counts, their recomputed request-local total, and bounded per-request work-kind tallies with a fixed association label. Never expose raw usage records, provider cache-control blocks or lifetime token breakdowns, provider message/session/event IDs, model identifiers, comparison groups, dedupe keys, provider totals, cumulative totals, prompts, or billing fields.
- Cache-read-drop metadata is separate from write-backed refill evidence. It may expose only readiness, normalized agent IDs, bounded counts, opaque occurrence IDs, original observation timestamps, preceding/current cache-read percentages, and elapsed gaps. Numeric eligibility and predecessor timestamps stay monitor-private. Present the refill interpretation as an inference, never a confirmed write, expiry, cause, cost, or savings claim; exclude it from write-backed event/report totals.
- Keep the monitor bound to loopback.
- Send OAuth credentials only to the provider's authenticated usage endpoint and, for locally registered Claude Remote Control sessions with a validated process owner, the read-only `GET https://api.anthropic.com/v1/code/sessions/{id}` metadata endpoint. This status exception never permits remote discovery, event reads, worker attachment, control actions, token refresh, or redirects. Keep remote IDs, credentials, payloads, and errors monitor-private; retain only normalized lifecycle and local observation time. See `docs/CLAUDE_SESSION_STATUS.md`.
...
- Desktop provider-folder settings are explicit native actions. Persist only three bounded absolute folder overrides in private desktop settings: Claude configuration, Claude session projects, and Codex home. Renderer IPC accepts only fixed folder keys and returns bounded selection/availability/action states, never paths. Native folder selection and save/restart confirmation own the actual paths. Browser/LAN APIs cannot configure sources. Apply one coherent profile to discovery, presence, tasks, account usage, and native provider setup; preserve checkpoint source validation and isolate default local usage/cost snapshots for custom Claude profiles. See `docs/OBSERVATION_CACHE.md`.
- Read-only **Settings → Providers** in a same-computer browser or authenticated paired LAN browser may display the three effective provider folder roots through the dedicated no-store `GET /api/provider-folders`, as explicitly approved by the user. Return only bounded absolute Claude configuration, Claude session projects, and Codex home roots, or null. This exception excludes transcript-file paths, credentials, ordinary normalized state, reports, logs, checkpoints, and browser persistence. Local development access requires an actual local socket peer and a recognized machine hostname or interface address; forwarded headers cannot authorize it. LAN gateway forwarding requires current pairing and network authorization. Deny unpaired remote access, cross-origin requests, writes, and path inputs. Serve startup configuration without provider acquisition; folder changes remain native actions.
- Desktop Claude Code reconnection and local usage setup are explicit user-confirmed native actions, outside monitor GETs. Reconnection invokes only the installed native CLI's sign-in flow; it never reads, refreshes, collects, or forwards credentials. Local usage setup changes only the user's status-line configuration after native confirmation and preserves the existing configured command. Neither action is exposed through HTTP or accepts commands, paths, or URLs from the renderer.
- Internal pipeline-operations diagnostics remain read-only, bounded, and outside browser API state. In normal development, startup creates one anonymous continuous JSONL writer before observation begins. It owns only `outputs/pipeline-logs/` files, retains at most ten 25 MiB files (250 MiB default), has a 1 MiB queued-record limit plus a separately bounded in-flight batch of at most 1 MiB, and assumes one development writer. Records use only the fixed normalized operations vocabulary, a fresh per-run UUID, timestamps, synthetic lanes and opaque numeric handles. They must never contain session IDs, source paths or fingerprints, prompts, responses, transcript/tool content, credentials, or raw errors. Explicit gap, loss, lifecycle, health, incomplete-span, and observed-coverage semantics prevent retention or absence from being presented as complete session history. `npm run diagnostics:logs -- ...` is a passive file-first analyzer/follower; it never connects to the monitor or triggers pipeline work. The rolling recorder, authenticated capture IPC, and Perfetto export/viewer remain optional targeted development tooling; preserve their existing private scope and contracts. GETs and production/desktop artifacts exclude all continuous logging, capture transport, and renderer instrumentation; normal development does not start capture IPC. See `docs/PIPELINE_OPERATIONS.md`.
- Browser `performance.mark()` telemetry is a development-only F Presentation diagnostic, not normalized session evidence or a product metric. It may use only fixed allowlisted marks and opaque revision tokens minted after committed responses, with bounded revision-correlated durations under the privacy contract in `docs/PIPELINE_OPERATIONS.md`. Production and desktop builds exclude the implementation and endpoint. Browser or LAN traffic cannot enable recording, mint arbitrary associations, provide a clock claim, paths, URLs, text, source/session/native IDs, or output destinations. A token-bearing request may return only its fixed browser-monotonic start/response interval; the monitor derives and exports only a bounded request-interval error, never raw browser timestamps or a browser-selected offset.
...
- Cache-read-drop inference requires explicit numeric provenance and adjacent comparable requests, a high-to-low cache-read-share transition, and an actual cached-token collapse. Missing or malformed evidence, compactions, context reductions, and model/comparison-group changes break comparisons. It must never relax the recorded-write requirement for refill classification or turn a minimum-only cache lifetime into an expiry threshold.
